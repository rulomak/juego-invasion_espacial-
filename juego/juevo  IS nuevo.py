...
# sirve para crear la pantala
pantalla = pygame.display.set_mode((800, 600))

# Titulo e icono
pygame.display.set_caption("Invasion Espacial")
# iconos los puedo bahar de flaticon.es / fondos freepik.com
...
#funcion jugados
def jugador(x,y):
    pantalla.blit(img_jugador,(x,y))  #.blit arrojar o incertar
#funcion enemigo
def enemigo(x,y,ene):
    pantalla.blit(img_enemigo[ene],(x,y))  #.blit arrojar o incertar

#funcion bala
def disparar_bala(x,y):
    global bala_visible
    bala_visible = True
    pantalla.blit(img_bala,(x + 16,y + 10)) # se le agrega estos pix para que salga del centro de la nave
...
# loop del juego
se_ejecuta = True
while se_ejecuta:                          # mientra que se ejecuta sea True por cada eventa  si evento es igual a QUIT
    ...
    for evento in pygame.event.get():
        ...
        if evento.type == pygame.KEYDOWN:   # tecla precionada (cualquira)
            if evento.key == pygame.K_LEFT:   #ese evento es igual a tecla izq
                jugador_x_cambio = -0.6
            if evento.key == pygame.K_RIGHT: #ese evento es igual a tecla der
                jugador_x_cambio = 0.6
            if evento.key == pygame.K_SPACE:
                sonido_bala = mixer.Sound('disparo.mp3')

                if bala_visible == False:  # solamente se diparar una vez y no cambiara la pasision cada ves que apretemos space
                    bala_x = jugador_x    # esto hace que la bala mantenga la posision de diparo y no corra junto al jugador
    ...
    jugador_x += jugador_x_cambio            # jugadorX (ancho) va a ser igual que jugadorX cambiao que estara cambiano
    #mantener dentro de margenes J
    if jugador_x <= 0:
        jugador_x = 0
    elif jugador_x >= 736:
        jugador_x = 736
